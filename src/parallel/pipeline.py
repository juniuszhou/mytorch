...
def manual_model_split(model) -> PipelineStage:
    # fist stage just run layer 0 to 3, and embedding
    if stage_index == 0:
        ...
        model.norm = None
        model.output = None
    # second stage just run layer 4 to 7, and norm and output
    elif stage_index == 1:
...
def tracer_model_split(model, example_input_microbatch) -> PipelineStage:
    pipe = pipeline(
        ...
        # set the split point for the model.
        split_spec={
...
if __name__ == "__main__":
    ...
    # split data into num_microbatches, and run the model in each stage
    schedule = ScheduleGPipe(
        stage, n_microbatches=num_microbatches, loss_fn=tokenwise_loss_fn
    )
    ...
    if rank == 0:
        ...
        losses = []
        # for rank 1, the input data is from stage 1.
